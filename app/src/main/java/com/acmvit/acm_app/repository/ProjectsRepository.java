...
public class ProjectsRepository {
    ...
    public PaginatedResource<Project> getProjectsWithName(
        @NonNull String name
    ) {
        return new PaginatedNetworkBoundResource<HashMap<String, List<Project>>, Project>(
            15,
            localDb
        ) {
            @Override
            protected Completable saveCallResult(
                HashMap<String, List<Project>> item
            ) {
            ...
            protected void createCall(
                MutableLiveData<Resource<HashMap<String, List<Project>>>> reqItem,
                int lastRequestedPage
            ) {
                tokenizedService
                    .getProjectsWithName(name, lastRequestedPage)
    ...
    public PaginatedResource<Project> getProjectsWithTag(@NonNull String tag) {
        return new PaginatedNetworkBoundResource<ProjectsList, Project>(
            15,
            localDb
        ) {
            @Override
            ...
            @Override
            protected void createCall(
                MutableLiveData<Resource<ProjectsList>> reqItem,
                int lastRequestedPage
            ) {
                tokenizedService
                    .getProjectsWithTag(tag, lastRequestedPage)
                    .enqueue(new BackendNetworkCall<>(reqItem));
            }
        }
            .get();
    ...
    public PaginatedResource<Project> getProjectsWithUidAndStatus(
        @Nullable String uid,
        ...
        return new PaginatedNetworkBoundResource<ProjectsList, Project>(
            ...
            protected DataSource.Factory<Integer, Project> loadFromDb() {
                return localDb
                    .projectDao()
                    .getAllProjectsWithUserStatus(status, uid);
            }
            ...
            protected void createCall(
                ...
                if (uid == null && status == null) {
                    tokenizedService
                        .getAllProjects(lastRequestedPage)
                        .enqueue(new BackendNetworkCall<>(reqItem));
                } else if (uid == null) {
                    tokenizedService
                        .getProjectsWithStatus(statusS, lastRequestedPage)
                        .enqueue(new BackendNetworkCall<>(reqItem));
                } else if (status == null) {
                    tokenizedService
                        .getProjectsWithUser(uid, lastRequestedPage)
                        .enqueue(new BackendNetworkCall<>(reqItem));
                } else {
                    tokenizedService
                        .getProjectsWithUserAndStatus(
                            uid,
                            statusS,
                            lastRequestedPage
                        )
        ...
        return new NetworkBoundResource<Map<String, List<Tag>>, List<String>>() {
            ...
            protected LiveData<List<String>> loadFromDb() {
                return Transformations.map(
                    localDb.tagDao().getAllTags(),
                    tagConverter::entityToModel
                );
            }

            @Override
            protected void createCall(
                MutableLiveData<Resource<Map<String, List<Tag>>>> reqItem
            ) {
                BackendNetworkCall<Map<String, List<Tag>>> call = new BackendNetworkCall<>(
                    reqItem
                );
    ...
    private Completable saveProjects(List<Project> projects) {
        ...
        Completable projectCompletable = localDb
            .projectDao()
            .insertProject(projectDbs);
        Completable artifactsCompletable = Observable
            .fromIterable(projects)
            .switchMapCompletable(
                project ->
                    localDb
                        .userDao()
                        .insertUsersAndIgnoreIfExists(
                        ...
                        .andThen(
                            localDb
                                .tagDao()
                                .insertTags(
                                    tagConverter.modelToEntity(
                                        project.getTags()
                                    )
                                )
                        )
                        .andThen(
                            localDb.projectMemberCrossRefDao().insert(project)
                        )
                        .andThen(
                            localDb.projectTagCrossRefDao().insert(project)
                        )
            );
