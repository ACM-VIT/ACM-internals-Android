...
public class ProjectsFragment extends Fragment {
    ...
    public void onViewCreated(
        ...
        savedStateHandle =
            navController
                .getBackStackEntry(R.id.projectsFragment)
                .getSavedStateHandle();
        MutableLiveData<User> userFilterSelected = savedStateHandle.getLiveData(
            MembersBottomDialogFragment.RESULT_KEY
        );
        userFilterSelected.observe(
    ...
    private void initObservers() {
        viewModel
            .getIsSearchExpanded()
            .observe(getViewLifecycleOwner(), this::toggleSearchUI);
        viewModel
            .getFilters()
            .getUser()
            .observe(
                getViewLifecycleOwner(),
                user -> binding.myChip.setChecked(user != null)
            );
        viewModel
            .getFilters()
            .getTagSearchFilter()
        ...
        viewModel
            ...
            .observe(
                getViewLifecycleOwner(),
                aVoid -> binding.projectsRv.scrollToPosition(0)
            );
    }

    private void initMyChip() {
        binding.myChip.setOnClickListener(
            v ->
                navController.navigate(
                    ProjectsFragmentDirections.actionProjectsFragmentToMembersBottomDialogFragment()
                )
        );
        binding.myChip.setOnCheckedChangeListener(
            (buttonView, isChecked) -> {
                boolean viewModelChecked =
                    viewModel.getFilters().getUser().getValue() != null;
                if (viewModelChecked != isChecked) {
                    buttonView.setChecked(viewModelChecked);
                } else {
                    binding.myChip.setChipIconSizeResource(
                        isChecked
                            ? R.dimen.my_chip_selected_icon_size
                            : R.dimen.my_chip_unselected_icon_size
                    );
                    binding.myChip.setTextEndPaddingResource(
                        isChecked
                            ? R.dimen.my_chip_selected_text_end_padding
                            : R.dimen.my_chip_unselected_text_end_padding
                    );
                    binding.myChip.setCloseIconVisible(isChecked);
                    if (isChecked) {
                        binding.myChip.setChipIconTint(null);
                    } else {
                        binding.myChip.setChipIconTintResource(
    ...
    private void initProjectsRv(RecyclerView rv) {
        ...
        ProjectsRvAdapter projectsAdapter = new ProjectsRvAdapter(
            viewModel.getProjectsPaginatedResource(),
            getViewLifecycleOwner()
        );
        projectsAdapter.registerAdapterDataObserver(
    ...
    private void initAnimations() {
        binding.searchTextfield.setOnFocusChangeListener(
            (v, hasFocus) -> {
                if (hasFocus) {
        ...
        binding.rootMl.setTransitionListener(
            new MLTransitionListener() {
                final float overlay_unfocused_elevation = getResources()
                    .getDimension(R.dimen.overlay_elevation_unfocused);
                ...
                public void onTransitionCompleted(
                    ...
                    if (
                        current == R.id.start &&
                        binding.overlayView.getElevation() !=
                        overlay_unfocused_elevation
                    ) {
                        binding.overlayView.setElevation(
                            overlay_unfocused_elevation
                        );
                        GeneralUtils.hideKeyboard(
                            getContext(),
                            binding.searchTextfield
                        );
                        binding.rootMl.setTransition(R.id.scroll_collapse_ts);
                    }
                }
    ...
    private void toggleSearchUI(boolean enable) {
        boolean isCurrentEnable =
            binding.rootMl.getCurrentState() == R.id.end_search_extend;
        if (isCurrentEnable == enable) return;

        binding.rootMl.setTransition(R.id.search_extend_ts);
        if (enable) {
            binding.overlayView.setElevation(
                getResources().getDimension(R.dimen.overlay_elevation_focus)
            );
    ...
    private void setSpan() {
        ...
        chip.setBounds(
            0,
            0,
            chip.getIntrinsicWidth(),
            search.getHeight() -
            search.getTotalPaddingTop() -
            search.getTotalPaddingBottom()
        );
        ...
        editable.setSpan(
            span,
            0,
            viewModel.getSearchData().getValue().length(),
    ...
    private void removeSpans() {
        ...
        ImageSpan[] toRemoveSpans = text.getSpans(
            0,
            text.length(),
            ImageSpan.class
        );
        for (ImageSpan toRemoveSpan : toRemoveSpans) text.removeSpan(
            toRemoveSpan
        );
    }
}
