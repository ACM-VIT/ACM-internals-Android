...
public class MembersBottomAdapter
    extends ListAdapter<User, MembersBottomAdapter.MembersVH> {

    private OnItemClickListener onItemClickListener = pos -> {};

    public MembersBottomAdapter() {
        super(
            new SimpleDiffCallback<User>() {
                @Override
    ...
    @NonNull
    @Override
    public MembersVH onCreateViewHolder(
        @NonNull ViewGroup parent,
        int viewType
    ) {
        LayoutInflater inflater = LayoutInflater.from(parent.getContext());
        ItemDialogMemberBinding binding = ItemDialogMemberBinding.inflate(
            inflater,
            parent,
            false
        );
